	...
	base.executeRest(req, res, function(err, req, res, conn){
		if (err) {
			...
			employeeDao.selectEmployeeById(req.params.uid, conn, function(err, rows){
				console.log('[getEmployee] selectEmployeeById callback');
				console.log('err:' + err);
		...
		if (err) {
			...
			if(type==="byall" || type==="bysn" || type==="byname" || type==="byphone"){
				employeeDao.selectEmployeeBy(query, type, conn, function(err, rows){
					console.log('[searchEmployee] selectEmployeeBy callback');
					console.log('err:' + err);
					console.log('rows: ' + rows);
					if(err){
						res.send({"status":"error","error":err});
	...
	base.executeRest(req, res, function(err, req, res, conn){
		if (err) {
			...
			cartDao.selectItemsInCartsByEmployee(eid, conn, function(err, rows) {
				console.log('[getCartsByEmployee] selectItemsInCartsByEmployee callback');
				console.log('err:' + err);
				console.log('rows: ' + rows);
				if (err) {
					res.send({"status":"error","error":err});
